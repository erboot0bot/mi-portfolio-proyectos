...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--bg)] text-[var(--text)]">
      <header
        ref={headerRef}
        className="sticky top-0 z-40 border-b border-[var(--border)] bg-[var(--nav-bg)] backdrop-blur-md relative"
        style={{ boxShadow: 'inset -160px 0 100px -80px rgba(154,78,251,0.2)' }}
      >
        <div className="w-full px-4 h-[60px] flex items-center justify-between">
          {/* Left: logo */}
          <Link to="/" aria-label="H3nky" className="flex items-center shrink-0" style={{ height: '40px' }}>
            <img
          ...
          {/* Right: github, language, theme, user */}
